...
namespace Broccoli.Builder {
	...
	public class BranchMeshBuilder {
				...
				int shapeVerticesStartAt)
			{
				...
				for (int i = segmentStartAt; i < segmentStartAt + segmentLength; i++) {
					...
					if (prevSegmentSideCount > 0 && 
						segmentType > BranchMeshBuilder.BranchSkin.SEGMENT_TYPE_BEGIN_CAP)
					{
				...
				int segmentBuilder)
			{
				...
				if (polygonSides >= 3) {
					...
					for (int i = 0; i <= polygonSides; i++) {
						...
						if (!useHardNormals) {
							if (segmentType == BranchMeshBuilder.BranchSkin.SEGMENT_TYPE_END_CAP) vertexNormal = direction_posAtSkin.normalized;
							else vertexNormal = (vertex - (Vector3)center_pos).normalized;
						}
		...
		public class BranchSkin {
			#region Consts
			public const int SEGMENT_TYPE_BEGIN		= 0;
			public const int SEGMENT_TYPE_BEGIN_CAP	= 1;
			...
			public BranchSkin (int minPolygonSides, int maxPolygonSides, float minAvgGirth, float maxAvgGirth, float lengthOffset, float hierarchyLevel, bool useHardNormals) {
				...
				int numberOfSegments, int segmentType, float girth, float position, float positionAtSkin, BuilderType builder)
			{
				...
				if (segmentType != SEGMENT_TYPE_BEGIN) {
		...
		public BranchSkin GetOrCreateBranchSkin (BroccoTree.Branch firstBranch, int level, float lengthOffset, float hierarchyLevel) {
			...
			BranchSkin branchSkin = new BranchSkin (minPolygonSides, maxPolygonSides, treeMinAvgGirth, treeMaxAvgGirth, lengthOffset, hierarchyLevel, useHardNormals);
			...
			if (ContainsMeshBuilder (BuilderType.Welding)) {
				WeldingMeshBuilder weldingMeshBuilder = (WeldingMeshBuilder) GetBranchMeshBuilder (BuilderType.Welding);;
				do {
