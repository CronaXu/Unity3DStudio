...
namespace Broccoli.Builder
{
	...
	public class STWindMetaBuilder {
		...
		struct BranchWindJob : IJobParallelFor {
			...
			/// w: Adds wind data: branch swing phase.
			/// </summary>
			...
			/// x: Adds wind data: vertex x position.
			/// y: Adds wind data: vertex y position.
			/// z: Adds wind data: vertex z position..
			/// w: Adds wind data: vertex z position on the branch origin..
			/// </summary>
			...
			public float maxBranchLengthFromTreeOrigin;
			public void Execute(int i) {
				...
				// Set UV
				uv.z = uv5.y / maxBranchLengthFromTreeOrigin; // Length from tree origin (0-1)
				uv.w = uv6.y; // Phase, id of the branch skin.
		...
		#region Vars
		float maxBranchLengthFromOrigin = 0f;
		/// <summary>
		...
		public void AnalyzeTree (BroccoTree tree, List<BranchMeshBuilder.BranchSkin> branchSkins) {
			var bsEnumerator = branchSkins.GetEnumerator ();
			maxBranchLengthFromOrigin = 0f;
			while (bsEnumerator.MoveNext ()) {
				if (bsEnumerator.Current.lengthOffset + bsEnumerator.Current.length > maxBranchLengthFromOrigin) {
					maxBranchLengthFromOrigin = bsEnumerator.Current.lengthOffset + bsEnumerator.Current.length;
				}
			}
		...
		#region Branch Mesh
		public void SetBranchesWindData (Mesh mesh) {
			...
			branchWindJob.maxBranchLengthFromTreeOrigin = maxBranchLengthFromOrigin;
			...
			branchWindJob.uv3s = new NativeArray<Vector4> (totalVertices, Allocator.TempJob);
			...
			branchWindJob.uv3s.CopyTo (_uv3s);
		...
		*/
		...
		public Vector4 GetUV (BroccoTree.Branch branch, float position, Vector4 originalUV, bool isSprout = false) {
			Vector2 uv = GetUV (branch, position);
